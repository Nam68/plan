...
public class PlaceService {
	...
	@Transactional
	public ErrorJudgment save(Place place, Member member) {
		if(place == null) return ErrorJudgment.ERROR;
	...
	@Transactional
	public void update(Place place, Place newPlace, Member member) {
		place.setTitle(newPlace.getTitle());
		place.setRegion(newPlace.getRegionEnum());
		place.setMemo(newPlace.getMemo());
		place.setRegisterDate(new Date(System.currentTimeMillis()));
		place.setMember(member);
	 ...
	 * 페이징된 장소 반환
	 */
	public List<Place> findAllWithPage(int page) {
		...
		List<Place> result = repository.findAll(pageRequest).getContent();
		return result;
	}
